...
def login_page():
    ...
    st.markdown("---")
    st.write("Hosted UI를 통해 로그인 및 회원가입이 가능합니다.")
    login_url = auth.get_login_url()
    st.link_button("🔑 로그인 / 회원가입", login_url, use_container_width=True)
    st.markdown("---")
    with st.expander("🔧 기술 정보"):
        st.write(f"Client ID: `{auth.client_id}`")
        st.write(f"Cognito Domain: `{auth.domain}`")
        st.write(f"Redirect URI: `{auth.redirect_uri}`")
